...
impl<'a> SpdxLicenseStore<'a> {
    ...
    /// Fetch the templates content from GitHub repository.
    pub async fn fetch_local_license_details<T>(&self, spdx_id: T) -> Result<SpdxLicenseDetails>
    where
        T: AsRef<str>,
    {
    ...
    /// Fetch the templates content from GitHub repository.
    pub async fn fetch_remote_license_details<T>(&self, spdx_id: T) -> Result<SpdxLicenseDetails>
    where
        T: AsRef<str>,
    {
        let SpdxLicenseMetadata { details_url, .. } = Self::get_license_metadata(&spdx_id)?;
